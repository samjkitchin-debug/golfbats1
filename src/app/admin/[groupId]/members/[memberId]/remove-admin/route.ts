...
/**
 * POST /admin/[groupId]/members/[memberId]/remove-admin
 * Set a member's role to "member" for the specified group (remove admin status).
...
export async function POST(
  ...
  try {
    ...
    if (targetMembership.status !== "approved") {
      return NextResponse.json(
        { error: "Can only modify admin role for approved members." },
        { status: 400 }
      );
    }
