...
export default function AdminMembersPage() {
  ...
  async function handleViewPassport(memberId: string) {
    ...
    try {
      ...
      const res = await fetch(`/admin/g/${groupId}/members/${memberId}/passport`, {
        method: "GET",
      });
  ...
  async function handleDeleteMember(memberId: string, memberName: string, isReject = false) {
    ...
    try {
      const res = await fetch(`/admin/g/${groupId}/members/${memberId}/delete`, {
        method: "DELETE",
      });
  ...
  async function handleApproveMember(memberId: string, memberName: string) {
    ...
    try {
      const res = await fetch(`/admin/g/${groupId}/members/${memberId}/approve`, {
        method: "POST",
      });
  ...
  async function handleMakeAdmin(memberId: string, memberName: string) {
    ...
    try {
      setApprovingMemberId(memberId);
      const res = await fetch(`/admin/g/${groupId}/members/${memberId}/make-admin`, {
        method: "POST",
      });
      const json = await res.json().catch(() => ({}));
      if (!res.ok) {
  ...
  async function handleRemoveAdmin(memberId: string, memberName: string) {
    ...
    try {
      setApprovingMemberId(memberId);
      const res = await fetch(`/admin/g/${groupId}/members/${memberId}/remove-admin`, {
        method: "POST",
      });
      const json = await res.json().catch(() => ({}));
      if (!res.ok) {
  ...
  async function handleSaveRoles() {
    ...
    try {
      ...
      if (rolesIsAdmin && editingRolesMember.group_role !== "admin") {
        // Promote to admin
        const res = await fetch(`/admin/g/${groupId}/members/${targetId}/make-admin`, { method: "POST" });
        const json = await res.json().catch(() => ({}));
        if (!res.ok) {
          throw new Error(json?.error || "Failed to make member admin.");
        }
        ...
        // Remove admin
        const res = await fetch(`/admin/g/${groupId}/members/${targetId}/remove-admin`, { method: "POST" });
        const json = await res.json().catch(() => ({}));
        if (!res.ok) {
          throw new Error(json?.error || "Failed to remove admin status.");
        }
  ...
  return (
    <main className="mx-auto w-full max-w-5xl px-4 pb-10">
      <div className="mt-6 flex items-center justify-between">
        ...
        <Link href={`/admin/g/${groupId}`} className="text-sm text-foreground hover:text-foreground">
          Back to dashboard
        </Link>
      </div>
