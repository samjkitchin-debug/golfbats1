...
type MemberRow = {
  ...
  // Group membership data (from group_members table)
...
export default function AdminMembersPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        // Step 1: Fetch group_members for this group
        ...
        // Step 2: Extract user_ids and fetch member details
        const userIds = groupMembersData.map((gm) => gm.user_id);
        const { data: membersData, error: membersError } = await supabase
          .from("members")
          .select("id, email, full_name, display_name, nationality, declared_handicap, created_at, last_seen, profile_photo_path")
          .in("id", userIds);
        ...
        // Step 3: Combine group_members and members data
        ...
        const sorted = memberRows.sort((a, b) => {
          const aIsAdmin = a.group_role === "admin";
          const bIsAdmin = b.group_role === "admin";
        ...
        // Load passport statuses (if API exists)
        try {
          const passportRes = await fetch("/api/admin/passport-statuses");
          const passportJson = await passportRes.json().catch(() => ({}));
          if (passportRes.ok && passportJson?.statuses) {
            setPassportStatuses(passportJson.statuses as Record<string, PassportStatus>);
          }
        } catch (passportErr) {
          // Passport statuses are optional - continue without them
        }
      } catch (err: unknown) {
  ...
  const metrics = useMemo(() => {
    const pending = members.filter((m) => m.group_status === "pending").length;
    ...
    const inactive = members.filter((m) => {
      const status = passportStatuses[m.id];
      const isApproved = m.group_status === "approved";
      return isApproved && !status?.hasPassport;
    }).length;
  ...
  const filteredMembers = useMemo(() => {
    ...
    if (activeTab === "pending") {
      ...
      filtered = filtered.filter((m) => m.group_role === "admin");
    } else if (activeTab === "members") {
      filtered = filtered.filter((m) => m.group_role === "member");
    }
  ...
  async function handleViewPassport(memberId: string) {
    ...
    try {
      ...
      const res = await fetch(`/admin/g/${group.slug}/members/${memberId}/passport`, {
        method: "GET",
      });
  ...
  async function handleDeleteMember(memberId: string, memberName: string, isReject = false) {
    ...
    try {
      const res = await fetch(`/admin/g/${group.slug}/members/${memberId}/delete`, {
        method: "DELETE",
      });
  ...
  async function handleApproveMember(memberId: string, memberName: string) {
    ...
    try {
      const res = await fetch(`/admin/g/${group.slug}/members/${memberId}/approve`, {
        method: "POST",
      });
  ...
  async function handleMakeAdmin(memberId: string, memberName: string) {
    ...
    try {
      setApprovingMemberId(memberId);
      const res = await fetch(`/admin/g/${group.slug}/members/${memberId}/make-admin`, {
        method: "POST",
      });
      const json = await res.json().catch(() => ({}));
      if (!res.ok) {
  ...
  async function handleRemoveAdmin(memberId: string, memberName: string) {
    ...
    try {
      setApprovingMemberId(memberId);
      const res = await fetch(`/admin/g/${group.slug}/members/${memberId}/remove-admin`, {
        method: "POST",
      });
      const json = await res.json().catch(() => ({}));
      if (!res.ok) {
  ...
  async function handleSaveRoles() {
    ...
    try {
      ...
      if (rolesIsAdmin && editingRolesMember.group_role !== "admin") {
        // Promote to admin
        const res = await fetch(`/admin/g/${group.slug}/members/${targetId}/make-admin`, { method: "POST" });
        const json = await res.json().catch(() => ({}));
        if (!res.ok) {
          throw new Error(json?.error || "Failed to make member admin.");
        }
      } else if (!rolesIsAdmin && editingRolesMember.group_role === "admin") {
        // Remove admin
        const res = await fetch(`/admin/g/${group.slug}/members/${targetId}/remove-admin`, { method: "POST" });
        const json = await res.json().catch(() => ({}));
        if (!res.ok) {
          throw new Error(json?.error || "Failed to remove admin status.");
        }
  ...
  return (
    <main>
      <div className="flex items-center justify-between">
        ...
        <Link href={`/admin/g/${group.slug}`} className="text-sm text-foreground hover:text-foreground">
          Back to dashboard
        </Link>
      </div>
                            ...
                            <tr key={m.id} className="border-b last:border-b-0 hover:bg-background">
                              ...
                              <td className="px-4 py-2.5">
                                <div className="flex items-center justify-end gap-2">
                                  <Link
                                    href={`/admin/g/${groupId}/members/${m.id}/passport`}
                ...
                <>
                  ...
                  <div className="block sm:hidden divide-y">
                    {filteredMembers.map((m) => {
                      ...
                      const isActive = m.group_status === "approved";
                                  ...
                                  <div className="py-1">
                                    ...
                                    <button
                                      onClick={() => {
                                        ...
                                        setRolesIsAdmin(m.group_role === "admin");
                                        setOpenMenuMemberId(null);
                                      }}
                                      className="w-full px-4 py-2.5 text-left text-sm text-foreground hover:bg-background"
                                    >
